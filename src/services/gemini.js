...
export const generateFastVisualConcept = async (inputs) => {
  return retryOperation(async () => {
    ...
    const prompt = `你是一位顶尖的《JOJO的奇妙冒险》替身设计专家。请根据以下用户特征，为一个新替身构思核心概念。
    
    用户特征:
    1. 引用: "${inputs.song}" (作为命名及外形灵感来源)
    2. 色调: "${inputs.color}"
    3. 特质: "${inputs.personality}"

    **核心任务：深度分析【特质】与【歌曲】，创造最具JOJO“奇妙感”的替身设计。**

    🎨 **形态设计指南（自由但有原则）**：
    1. **优先考虑人形/半人形 (60%以上)**：这是JOJO最经典的形态（如白金之星、疯狂钻石、杀手皇后），最适合表现“精神力”和“战斗意志”。即使是抽象特质，也可以通过**异形化的人形**（如身体是发条、面部是屏幕、手脚是磁铁）来表达。
    2. **打破常规**：如果歌曲或特质有从“物理形态”到“概念形态”的强烈暗示，可以设计为**非人形**（如航空史密斯的飞机、性感手枪的子弹群、格林·D的婴儿）。但不要仅仅因为“贪婪”就无脑选群体型——也许是一个全身长满**拉链口袋的怪人**？
    3. **融合元素**：将【歌曲意象】与【特质】物理化。例如：歌曲是“海洋”，特质是“沉默”，可以设计一个“穿着潜水服的幽灵人形”。
    
    ❌ **避免刻板印象**：不要做简单的连线题（如“暴力必是壮汉”、“防御必是盾牌”）。请用**隐喻**和**艺术夸张**来设计！

    要求：
    1. 名字必须符合 JOJO 风格（源自欧美音乐/乐队名）。
    2. 外貌描述要包含基于'${inputs.color}'的独特配色。强调**荒木飞吕彦风格**（时尚姿势、肌肉线条、机械感与生机感的融合，奇特的装饰物）。
    3. **头部设计**：
       - 如果是人形，不要画成普通人脸！请参考**古典雕塑(希腊像)**、**机械面具**、**无机质外壳**或**抽象几何体**。
       - 除非明确是群体型/宠物型，否则不要设计卡通脸。

    只需返回以下 JSON (严禁 Markdown 代码块):
    {
      "reasoning": "简短分析：性格[坚毅]，歌曲[钢铁之心]暗示了金属质感。虽然特质是防御，但我决定设计一个【穿着重甲的近距离力量型】替身，而不是穿戴型，因为...",
      "name": "替身名 (中英文)",
      "appearance": "[类型：如近距离力量型] 详细外貌描述..."
    }`;
